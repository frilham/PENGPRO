...
func main() {
	...
	for decision == 0 && nBoneyard >= 4 {
		...
		// menampilkan jumlah Tile di boneyard
		fmt.Println(playerTiles, "Player")
...
func getDecision(decision *int) {
	...
	// KERJAKAN MANDIRI
	fmt.Scan(&decision)
	if *decision != 0 || *decision != 1 || *decision != 2 || *decision != 9 {
		fmt.Scan(&decision)
	}
}
...
func bagiTile(T, p *arrDomino, n *int) {
	...
	// KERJAKAN MANDIRI
	for i := 0; i < *n; i++ {
		p[i].left, p[i].right = T[i].left, p[i].right
		*n--
	}
}
...
func replaceTile(T, p *arrDomino, n *int, posisi int) {
	...
	// KERJAKAN MANDIRI
	for i := 0; i < *n; i++ {
		T[posisi].left, T[posisi].right = p[i].left, p[i].right
	}
}
